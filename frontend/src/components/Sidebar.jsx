      ...
      >
        {(isMobile || isTablet) && (
          <div className="flex items-center justify-between p-4 border-b border-slate-700">
            ...
            <button onClick={toggleSidebar} className="p-2 text-slate-400 hover:text-slate-300" aria-label="Close sidebar">
              <X size={18} />
            </button>
          </div>
        )}
        ...
        {/* Header label */}
        <div className="p-4">
          <div className="text-xs sm:text-sm text-slate-500 mb-2">MAIN MENU</div>
        </div>
        ...
        <nav className="flex-1 px-2 pb-4 overflow-y-auto">
          <div className="space-y-1">
            {menuItems.map((item, index) => (
              <div key={index}>
                {item.subItems ? (
                  <>
                    <div className="text-xs sm:text-sm text-slate-500 px-3 py-2 mt-4 uppercase tracking-wider">{item.name}</div>
          ...
          <div className="text-xs sm:text-sm text-slate-500 px-3 py-2 uppercase tracking-wider">Settings</div>
          ...
          <button
            ...
            className="mt-2 w-full flex items-center space-x-3 px-3 py-2 rounded text-sm text-slate-400 hover:bg-slate-800"
          >
